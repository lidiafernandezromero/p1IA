...
class Aichess():
    ...
    # MAIN HEURISTIC function h() that combines the two heuristics
    # takes into account all 5 possible checkmate positionns 
    def h(self, state):
        #possible positions for checkmate
        possCheckMates = [[[0,0,2],[2,4,6]],[[0,1,2],[2,4,6]],[[0,2,2],[2,4,6]],[[0,6,2],[2,4,6]],[[0,7,2],[2,4,6]]]

        #value to compare to keep the best heuristic found
        bestHeuristic = float('inf')

        #get the positions of the pieces in the current state
        rook = self.getPieceState(state, 2)
        king = self.getPieceState(state, 6)

        for checkMate in possCheckMates:

            #get the positions of the pieces in the checkmate possibilities
            goalRook = self.getPieceState(checkMate, 2)
            goalKing = self.getPieceState(checkMate, 6) 

            #calculate the heuristic for both king and rook
            hRook = self.hRook(rook, goalRook)
            hKing = self.hKing(king, goalKing)

            #combine the heuristics
            heuristic = hRook + hKing

            #update the best heuristic found
